...
v0.3.9 patches:
- Model alias compatibility: deepseek-chat and deepseek-reasoner now accepted as
  input aliases (map to v4-flash and v4-pro respectively) for backward compat with
  configs written before the July 24 2026 deprecation cutoff.
- Selective thinking field: the thinking payload field is now only sent to DeepSeek
  when thinking is actually enabled. Sending thinking={type:disabled} to deepseek-v4-flash
  in non-thinking mode caused unnecessary 400 errors on some API versions.
- Graceful model error recovery: 400/404 from DeepSeek on model-not-found now logs
  a clear diagnostic instead of forwarding a raw upstream error blob.
- Version sync: /health now reports 0.3.9, VERSION file updated.
- install.sh fallback version bumped to match current release.

v0.3.8 patches (prior):
...
MODEL_ALIASES: Dict[str, Tuple[str, bool]] = {
    ...
    # Legacy aliases: deepseek-chat and deepseek-reasoner were retired July 24 2026.
    # Accept them as input so old configs keep working; map to V4 equivalents.
    "deepseek-chat": ("deepseek-v4-flash", False),
    "deepseek-reasoner": ("deepseek-v4-flash", True),
    "deepseek@deepseek-chat": ("deepseek-v4-flash", False),
    "deepseek@deepseek-reasoner": ("deepseek-v4-flash", True),
}
...
def normalize_model(model: str) -> Tuple[str, str, bool]:
    ...
    if raw in MODEL_ALIASES:
        ...
        # For non-thinking base aliases, respect the env override
        if raw in {
            "deepseek@deepseek-v4-flash", "deepseek@deepseek-v4-pro",
            "deepseek-v4-flash", "deepseek-v4-pro",
            "deepseek-chat",  # legacy: maps to flash non-thinking but env can override
        }:
            thinking = default_thinking
        # Log a clear notice when a legacy alias is remapped
        if raw in {"deepseek-chat", "deepseek-reasoner",
                   "deepseek@deepseek-chat", "deepseek@deepseek-reasoner"}:
            log(
                f"legacy model alias remapped: {raw!r} -> deepseek@{ds_model} "
                "(deepseek-chat/reasoner retired July 24 2026; using V4 equivalent)"
            )
    ...
    raise ValueError(
        ...
        "deepseek@deepseek-v4-flash, deepseek@deepseek-v4-pro. "
        "Legacy aliases deepseek-chat and deepseek-reasoner are also accepted (mapped to V4 Flash)."
    )
...
def deepseek_payload(
    ...
    # Only send the thinking field when thinking is actually enabled.
    # Sending thinking={type: disabled} to deepseek-v4-flash in standard (non-thinking)
    # mode triggers spurious 400 errors on certain DeepSeek API versions. Omitting the
    # field entirely is safe and equivalent to disabled for non-thinking models.
    if thinking_enabled:
...
async def models() -> Dict[str, Any]:
    return {
        ...
        "data": [
            ...
            # Legacy aliases: accepted as input but deprecated. Listed so clients
            # that enumerate /v1/models can see them and migrate to V4 IDs.
            {"id": "deepseek-chat", "object": "model", "owned_by": "deepseek",
             "deprecated": True, "maps_to": "deepseek@deepseek-v4-flash"},
    ...
    if not stream:
        ...
        async with httpx.AsyncClient(timeout=HTTP_TIMEOUT) as client:
            ...
            if resp.status_code >= 400:
                ...
                # Provide a clear diagnostic for model-not-found errors (common after
                # the July 24 2026 legacy-name retirement and during V4-Pro GA rollout).
                if resp.status_code in (400, 404) and any(
                    kw in detail.lower()
                    for kw in ("model", "not found", "invalid_model", "does not exist")
                ):
                    log(
                        f"model-not-found diagnostic: requested model={deepseek_model!r}. "
                        "If using a legacy alias (deepseek-chat/deepseek-reasoner), "
                        "switch to deepseek-v4-flash or deepseek-v4-pro. "
                        "If using deepseek-v4-pro and seeing 400, the model may not yet "
                        "be GA on your account — try deepseek-v4-flash instead."
                    )
            ...
            async with httpx.AsyncClient(timeout=HTTP_TIMEOUT) as client:
                async with client.stream(
                    ...
                    if resp.status_code >= 400:
                        ...
                        # Provide a clear diagnostic for model-not-found errors.
                        if resp.status_code in (400, 404) and any(
                            kw in detail.lower()
                            for kw in ("model", "not found", "invalid_model", "does not exist")
                        ):
