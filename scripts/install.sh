...
if [ -z "${CODESEEQ_RELEASE_TAG+x}" ]; then
  if [ "$ALLOW_LATEST" != "true" ]; then
    die "CODESEEQ_ALLOW_LATEST_RELEASE is not true and CODESEEQ_RELEASE_TAG is unset; set CODESEEQ_RELEASE_TAG to a pinned release (e.g. v0.4.4) or set CODESEEQ_ALLOW_LATEST_RELEASE=true"
  fi
  ...
  RELEASE_TAG="$(curl -fsSL "$VERSION_URL" 2>/dev/null || echo "v0.4.4")"
